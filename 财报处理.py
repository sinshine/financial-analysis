...
df5.loc['其他非流动资产']=df5.loc['非流动资产合计']-df5.loc['可供出售金融资产']-df5.loc['持有至到期投资']-df5.loc['投资性房地产']-df5.loc['长期股权投资']-df5.loc['长期应收款']-df5.loc['固定资产']-df5.loc['在建工程']-df5.loc['工程物资']-df5.loc['固定资产清理']-df5.loc['生产性生物资产']-df5.loc['油气资产']-df5.loc['无形资产']-df5.loc['开发支出']-df5.loc['商誉']-df5.loc['长期待摊费用']-df5.loc['递延所得税资产']

df5.loc['其他流动负债']=df5.loc['流动负债合计']-df5.loc['短期借款']-df5.loc['交易性金融负债']-df5.loc['应付票据']-df5.loc['应付账款']-df5.loc['预收账款']-df5.loc['应付职工薪酬']-df5.loc['应交税费']-df5.loc['应付利息']-df5.loc['应付股利']-df5.loc['其他应付款']-df5.loc['预提费用']-df5.loc['预计负债']-df5.loc['一年内到期的非流动负债']

df5.loc['其他非流动负债']=df5.loc['非流动负债合计']-df5.loc['长期借款']-df5.loc['应付债券']-df5.loc['长期应付款']-df5.loc['专项应付款']-df5.loc['递延所得税负债']

df5.loc['负债总额']=df5.loc['流动负债合计']+df5.loc['非流动负债合计']
df5=df5.round(2)
...
#偿债能力指标
df0.loc['资产负债率']=df5.loc['负债总额']/df5.loc['资产总计']
...
df0.loc['利息保障倍数']=(df6.loc['三、利润总额（亏损总额以“－”号填列）']+df6.loc['　　财务费用（收益以“-”号填列）'])/df6.loc['　　财务费用（收益以“-”号填列）']

#经营周转指标
df0.loc['应收账款周转率（次）']=df6.loc['一、营业收入']*2/(df5.loc['应收账款']+df5.loc['应收账款'].shift(1))
df0.loc['存货周转率（次）']=df6.loc['减：营业成本']*2/(df5.loc['存货']+df5.loc['存货'].shift(1))
df0.loc['应付账款周转率（次）']=df6.loc['减：营业成本']*2/(df5.loc['应付账款']+df5.loc['应付账款'].shift(1))
df0.loc['预收账款周转率（次）']=df6.loc['一、营业收入']*2/(df5.loc['预收账款']+df5.loc['预收账款'].shift(1))
df0.loc['预付账款周转率（次）']=df6.loc['减：营业成本']*2/(df5.loc['预付账款']+df5.loc['预付账款'].shift(1))
...
df0.loc['收现率']=df7.loc['销售商品、提供劳务收到的现金']/df6.loc['一、营业收入']

#盈利指标
df0.loc['成本占比']=df6.loc['减：营业成本']/df6.loc['一、营业收入']
df0.loc['毛利润率']=1-df0.loc['成本占比']
df0.loc['营业利润率']=df6.loc['二、营业利润（亏损以“－”号填列）']/df6.loc['一、营业收入']
df0.loc['销售利润率']=(df6.loc['一、营业收入']-df6.loc['减：营业成本']-df6.loc['　　营业税金']-df6.loc['　　销售费用'])/df6.loc['一、营业收入']
df0.loc['净利润率']=df6.loc['四、净利润（净亏损以“－”号填列）']/df6.loc['一、营业收入']
df0.loc['EBIT']=(df6.loc['四、净利润（净亏损以“－”号填列）']+df6.loc['减：所得税']+df6.loc['　　　　其中：利息支出（减利息收入）'])/10000
df0.loc['总资产收益率']=df6.loc['四、净利润（净亏损以“－”号填列）']*2/(df5.loc['资产总计']+df5.loc['资产总计'].shift(1))
df0.loc['净资产收益率']=df6.loc['四、净利润（净亏损以“－”号填列）']*2/(df5.loc['所有者权益合计']+df5.loc['所有者权益合计'].shift(1))

#报表勾稽关系
df0.loc['报表所得税纳税比例']=df6.loc['减：所得税']/df6.loc['三、利润总额（亏损总额以“－”号填列）']
df0.loc['当年利润与年初年末未分配利润差额的匹配情况']=df5.loc['未分配利润'].shift(1)+df6.loc['四、净利润（净亏损以“－”号填列）']-df5.loc['未分配利润']
...
'''
list51=paixu(['交易性金融资产','应收票据','应收账款','预付账款','应收股利','应收利息','其他应收款','存货','待摊费用','一年内到期的非流动资产','其他流动资产'],df5)
...
#非流动资产排序
dict52={'可供出售金融资产':df5.loc['可供出售金融资产'].sum().round(1),'持有至到期投资':df5.loc['持有至到期投资'].sum().round(1),'投资性房地产':df5.loc['投资性房地产'].sum().round(1),'长期股权投资':df5.loc['长期股权投资'].sum().round(1),'长期应收款':df5.loc['长期应收款'].sum().round(1),'固定资产':df5.loc['固定资产'].sum().round(1),'在建工程':df5.loc['在建工程'].sum().round(1),'工程物资':df5.loc['工程物资'].sum().round(1),'固定资产清理':df5.loc['固定资产清理'].sum().round(1),'生产性生物资产':df5.loc['生产性生物资产'].sum().round(1),'油气资产':df5.loc['油气资产'].sum().round(1),'无形资产':df5.loc['无形资产'].sum().round(1),'开发支出':df5.loc['开发支出'].sum().round(1),'商誉':df5.loc['商誉'].sum().round(1),'长期待摊费用':df5.loc['长期待摊费用'].sum().round(1),'递延所得税资产':df5.loc['递延所得税资产'].sum().round(1),'其他非流动资产':df5.loc['其他非流动资产'].sum().round(1)}
...
#负债排序
dict53={'短期借款':df5.loc['短期借款'].sum().round(1),'交易性金融负债':df5.loc['交易性金融负债'].sum().round(1),'应付票据':df5.loc['应付票据'].sum().round(1),'应付账款':df5.loc['应付账款'].sum().round(1),'预收账款':df5.loc['预收账款'].sum().round(1),'应付职工薪酬':df5.loc['应付职工薪酬'].sum().round(1),'应交税费':df5.loc['应交税费'].sum().round(1),'应付利息':df5.loc['应付利息'].sum().round(1),'应付股利':df5.loc['应付股利'].sum().round(1),'其他应付款':df5.loc['其他应付款'].sum().round(1),'预提费用':df5.loc['预提费用'].sum().round(1),'预计负债':df5.loc['预计负债'].sum().round(1),'一年内到期的非流动负债':df5.loc['一年内到期的非流动负债'].sum().round(1),'其他流动负债':df5.loc['其他流动负债'].sum().round(1),'长期借款':df5.loc['长期借款'].sum().round(1),'应付债券':df5.loc['应付债券'].sum().round(1),'长期应付款':df5.loc['长期应付款'].sum().round(1),'专项应付款':df5.loc['专项应付款'].sum().round(1),'递延所得税负债':df5.loc['递延所得税负债'].sum().round(1),'其他非流动负债':df5.loc['其他非流动负债'].sum().round(1)}
list53=sorted(dict53.items(),key=lambda item:item[1])
df8.loc['总负债']=df5.loc['负债总额']
...
#所有者权益排序
dict54={'实收资本（或股本）':df5.loc['实收资本（或股本）'].sum().round(1),'资本公积':df5.loc['资本公积'].sum().round(1),'盈余公积':df5.loc['盈余公积'].sum().round(1),'一般风险准备':df5.loc['一般风险准备'].sum().round(1),'未分配利润':df5.loc['未分配利润'].sum().round(1),'外币报表折算差额':df5.loc['外币报表折算差额'].sum().round(1)}
...
#盈利能力
df9.loc['营业收入']=df6.loc['一、营业收入'].round(1)
df9.loc['营业利润']=df6.loc['二、营业利润（亏损以“－”号填列）'].round(1)
df9.loc['净利润']=df6.loc['四、净利润（净亏损以“－”号填列）'].round(1)
...
list56=paixu(['资产总计','货币资金','交易性金融资产','应收票据','应收账款','预付账款','应收股利','应收利息','其他应收款','存货','待摊费用','一年内到期的非流动资产','其他流动资产','可供出售金融资产', '持有至到期投资', '投资性房地产', '长期股权投资', '长期应收款', '固定资产', '在建工程', '工程物资', '固定资产清理', '生产性生物资产', '油气资产', '无形资产', '开发支出', '商誉', '长期待摊费用', '递延所得税资产', '其他非流动资产'],df5)
...
t2="从债务期限结构看，借款人近三期流动负债分别为%s、%s和%s万元，流动负债占比分别为%.2f%%、%.2f%%和%.2f%%。总体来看，公司偿债能力【较大\在合理范围内\较小，根据实际情况自行评价】。"  % (df5.loc['流动负债合计'][-3],df5.loc['流动负债合计'][-2],df5.loc['流动负债合计'][-1],df5.loc['流动负债合计'][-3]/df5.loc['负债总额'][-3]*100,df5.loc['流动负债合计'][-2]/df5.loc['负债总额'][-2]*100,df5.loc['流动负债合计'][-1]/df5.loc['负债总额'][-1]*100,)
...
#现金流量情况
t4="借款人近三期分别实现净现金流入%s、%s和%s万元。其中：经营活动现金净流入%s、%s和%s万元；投资活动现金净流入%s、%s和%s万元；筹资活动现金净流入%s、%s和%s万元。总体来看，借款人现金流情况【请根据实际情况自行评价】" % (df7.loc['期末净现金流量'][-3].round(1),df7.loc['期末净现金流量'][-2].round(1),df7.loc['期末净现金流量'][-1].round(1),df9.loc['经营活动净现金流'][-3],df9.loc['经营活动净现金流'][-2],df9.loc['经营活动净现金流'][-1],df9.loc['投资活动净现金流'][-3],df9.loc['投资活动净现金流'][-2],df9.loc['投资活动净现金流'][-1],df9.loc['筹资活动净现金流'][-3],df9.loc['筹资活动净现金流'][-2],df9.loc['筹资活动净现金流'][-1],)
